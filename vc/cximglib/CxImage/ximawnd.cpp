...
HICON CxImage::MakeIcon(HDC hdc, bool bTransparency)
{
	...
	// ***** MICSTO 2019/05/22 *****
	DeleteObject(csDest.hbmColor);
	DeleteObject(csDest.hbmMask);
	// ***** END MICSTO *****
...
bool CxImage::CreateFromHBITMAP(HBITMAP hbmp, HPALETTE hpal, bool bTransparency)
{
	...
	if (hbmp) { 
		...
		else
		{
			...
			if (!Create(bm.bmWidth, bm.bmHeight, bm.bmBitsPixel, GetType()))
			// END MICSTO
				return false;
...
#endif //_WIN32_WCE
////////////////////////////////////////////////////////////////////////////////
int32_t CxImage::Draw(HDC hdc, const RECT& rect, RECT* pClipRect, /*MICSTO*/ HPALETTE hPal, /*END MISCTO*/ bool bSmooth, bool bFlipY)
{
	return Draw(hdc, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, pClipRect, /*MICSTO*/ hPal, /*END MISCTO*/ bSmooth, bFlipY);
}
////////////////////////////////////////////////////////////////////////////////
/**
 ...
 * \return true if everything is ok
 */
int32_t CxImage::Draw(HDC hdc, int32_t x, int32_t y, int32_t cx, int32_t cy, RECT* pClipRect, /*MICSTO*/ HPALETTE hPal, /*END MISCTO*/ bool bSmooth, bool bFlipY)
{
	...
	*/
	...
	if (pClipRect)
	{
		paintbox.left = min(pClipRect->right, max(pClipRect->left, x));
		paintbox.right = max(pClipRect->left, min(pClipRect->right, x + cx));
		paintbox.top = min(pClipRect->bottom, max(pClipRect->top, y));
		paintbox.bottom = max(pClipRect->top, min(pClipRect->bottom, y + cy));
		bClip = TRUE;
	}
	else
	{
	...
	UINT uiPalColorsMapped = 0;
	if (hPal)
	{
		hPalOld = SelectPalette(hdc, hPal, TRUE);
		uiPalColorsMapped = RealizePalette(hdc);
	}
	// END MICSTO

	if (!(bTransparent || bAlpha || info.bAlphaPaletteEnabled)){
		if (cx == head.biWidth && cy == head.biHeight /*MICSTO*/ && !bClip /*END MICSTO */){ //NORMAL
#if !defined (_WIN32_WCE)
			...
			// MICSTO
			HPALETTE hPalOldTmp = NULL;
			UINT uiPalColorsMappedTmp = 0;
			if (hPal)
			{
				hPalOldTmp = SelectPalette(TmpDC, hPal, TRUE);
				uiPalColorsMappedTmp = RealizePalette(TmpDC);
			}
			// END MICSTO
			...
			// MICSTO
			if (hPalOldTmp)
				SelectPalette(TmpDC, hPalOldTmp, TRUE);
			// END MICSTO
...
int32_t CxImage::LayerDrawAll(HDC hdc, int32_t x, int32_t y, int32_t cx, int32_t cy, RECT* pClipRect, bool bSmooth)
{
	...
	while(pLayer=GetLayer(n++)){
		if (pLayer->Draw(hdc, x + pLayer->info.xOffset, y + pLayer->info.yOffset, cx, cy, pClipRect,/*MICSTO*/ NULL, /*END MICSTO*/bSmooth) == 0)
			return 0;
